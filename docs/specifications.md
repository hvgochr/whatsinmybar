...
`SameSite=Strict` and is `Secure` in production. Refresh tokens rotate on refresh with a fixed 10-second repeat window for
concurrent requests (see `docs/api.md`); their values are omitted from JSON responses. Refresh
